...
Move stringToMove(const std::string &str, const Board &board) {
	const int from = getBoardIndex(str[0], str[1]);
	const int to = getBoardIndex(str[2], str[3]);

	int flags = 0;

	const char piece = board.pieceAtSquare(from);
	const char target = board.pieceAtSquare(to);
